...
def computeLikelihoodGivenSeedAndFixedParams(galaxy, seed_params, seed_x, seed_y,
                                             param_initializer, pops, pop_selection_method, apply_observation_mask, halo_type,
    ...
    mask_z = mask_R[:]
    shared_object_holder = ssoh.SharedObjects(dist, mask_R, mask_z, np.linspace(-1.0, 1.0, n_los_bins), target_n_sky_pixels, max_sky_angle_deg * deg_to_rad, compute_mask = apply_observation_mask)
    ...
    for i in range(len(pops)):
        ...
        storer =  sgps.DwarfGalaxyParametersStorer(population, M = param_initializer['M'], rs = param_initializer['rs'],
                                                   halo_center = [param_initializer['h_x_center'], 0.0, param_initializer['h_z_center']],
                                                   sigsqr_rr_0 = param_initializer['sigsqr_rr_0' + pop_connector + pop], sigsqr_rr_inf_to_0_rat = param_initializer['sigsqr_rr_inf_to_0_rat' + pop_connector + pop], r_sigsqr_rr0 = param_initializer['r_sigsqr_rr0' + pop_connector + pop], alpha_sigsqr_rr = param_initializer['alpha_sigsqr_rr' + pop_connector + pop],
                                                   gamma_for_beta_inf = param_initializer['gamma_for_beta_inf' + pop_connector + pop], r_beta0 = param_initializer['r_beta0' + pop_connector + pop], alpha_beta = param_initializer['alpha_beta' + pop_connector + pop], stellar_data = star_data)

        parameter_storers = parameter_storers + [storer]

        surface_profile = ssbp.SurfaceBrightnessProfile(storer, shared_object_holder, halo_type, include_kinem_prob = include_kinem_prob, include_morph_prob = include_morph_prob)
...
def computeBestFitParamsAlongCurve(results_files, galaxy, fixed_params, halo_type, n_los_bins, target_n_sky_pixels, mask_R_bins,
                                   results_dir = '/Users/sashabrownsberger/Documents/Harvard/physics/randall/MCMCOutputs/', halo_funct = None, disk_funct = None,
                                   pops = ['MP','IM','MR'], pop_connector = '_', include_kinem_prob = 1, include_morph_prob = 1,
                                   pop_selection_method = 'metal_rigid', fit_funct_type = 'spline', data_type = 'real', apply_observation_mask = 1, dist = None,
    ...
    param_display_vals = msmr.getParamDisplayVals(pops = pops)
    var_display_scalings = msmr.getVarDisplayScalings(pops = pops)
    ...
    default_params = {'sigsqr_rr_0':100.0, 'sigsqr_rr_inf_to_0_rat':1.0, 'r_sigsqr_rr0':1000.0, 'alpha_sigsqr_rr':1.0,
                      'gamma_for_beta_inf':0.0, 'r_beta0':1000.0, 'alpha_beta':1.0,  }

    pop_dependent_vars = msmr.getVarPopDependence()
    ...
    fit_xs, fit_ys = msmr.fitMCMCResults(results_files, curve_params, pops = pops, measured_arrays = None, n_visits_array = None, param_ranges_to_fit = [x_range_to_check, y_range_to_check],
    ...
    print ('[seed_xs, seed_ys] = ' + str([seed_xs, seed_ys]))
    ...
    while not(max_found) and ((n_measured_points < len(seed_xs) ) or go_beyond_MCMC):
        ...
        start_likelihood = computeLikelihoodGivenSeedAndFixedParams(galaxy, curve_params, seed_x, seed_y,
                                                                    param_initializer, pops, pop_selection_method, apply_observation_mask, halo_type,
                                                                    dist, gamma, n_los_bins, target_n_sky_pixels, mask_R_bins,
        ...
        funct_to_minimize = lambda best_fit_x: -1.0 * computeLikelihoodGivenSeedAndFixedParams(galaxy, curve_params, best_fit_x, seed_y + fit_lines_slope * (best_fit_x - seed_x),
                                                                                               param_initializer, pops, pop_selection_method, apply_observation_mask, halo_type,
                                                                                               dist, gamma, n_los_bins, target_n_sky_pixels, mask_R_bins,
                                                                                               fixed_hx_rs_val, fixed_hz_rs_val, fixed_dx_rs_val, fixed_dz_rs_val, star_data_sets, pop_connector = pop_connector,
                                                                                               include_kinem_prob = include_kinem_prob, include_morph_prob = include_morph_prob )
    ...
    funct_to_minimize = lambda x_y_point: -1.0 * computeLikelihoodGivenSeedAndFixedParams(galaxy, curve_params, x_y_point[0], x_y_point[1] * 1.0 / y_scaling_for_minim,
                                                                                          param_initializer, pops, pop_selection_method, apply_observation_mask, halo_type,
                                                                                          dist, gamma, n_los_bins, target_n_sky_pixels, mask_R_bins,
                                                                                          fixed_hx_rs_val, fixed_hz_rs_val, fixed_dx_rs_val, fixed_dz_rs_val, star_data_sets, pop_connector = pop_connector,
                                                                                          include_kinem_prob = include_kinem_prob, include_morph_prob = include_morph_prob )
