...
#        return np.prod(self.onSkyInterpolator(np.dstack((corr_Ra_in_scale_radii,corr_Dec_in_scale_radii))[0]))
    ...
    def getProbAtSeveralPositions(self, sky_coords, dist_loses, v_loses, compute_vel_prob = 1, compute_pos_prob = 1):
        ...
        if compute_pos_prob and self.include_morph_prob:
            #start = time.time()
            #print ('[R_halos / self.rs, z_halos / self.rs, R_disks / self.rs, z_disks / self.rs, mass_density_exponent_scaling] = ' + str([R_halos / self.rs, z_halos / self.rs, R_disks / self.rs, z_disks / self.rs, mass_density_exponent_scaling]))
    ...
    def getNormalizationConstant(self):
        ...
        #print ('self.mask_vals = ' + str(self.mask_vals))
        #print ('unnormalized_surface_prob_array = ' + str(unnormalized_surface_prob_array))
    ...
    def calculateProbOnPhaseSpaceGrid(self, sky_coords, v_los_to_calc, solid_angles = [0.1], v_los_bin_widths = [1.0], compute_pos_prob = 1, compute_vel_prob = 1):
        ...
        #print ('[los_dist_edges, n_los_bins, los_dist_integrand_elems] = ' + str([los_dist_edges, n_los_bins, los_dist_integrand_elems]))
        ...
        #print ('[solid_angles, v_los_bin_widths, los_dist_integrand_elems, raw_prob_array] = ' + str([solid_angles, v_los_bin_widths, los_dist_integrand_elems, raw_prob_array]))
