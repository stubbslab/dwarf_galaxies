...
from samplingFunctions import varyVector 
...
class DwarfGalaxyParametersStorer: 
    ...
    def getMCMCInfo(self):
        ...
        if self.withDisk: 
            ...
            params_to_vary = ['rs', 'halo_sym_axis', 'el', 'zeta', 'dist_sym_axis', 'eps',  'lam']#set params to vary in case of disk + halo
            ...
            param_functions = [lambda scaler, jump_multiplier: constrainedGauss (scaler, 10.0 * jump_multiplier,[200.0,5000.0] ), #rs
                               lambda array,  jump_multiplier: varyVector (array, abs(constrainedGuass(0.0,0.05 * jump_multiplier))), # halo_sym_axis
                               lambda scaler, jump_multiplier: constrainedGauss(scaler, 0.01 * jump_multiplier,[0.1, 10.0] ), #el
                               lambda scaler, jump_multiplier: constrainedGauss(scaler, 0.01 * jump_multiplier,[0.00001, 1.0] ), #zeta
                               lambda scaler, jump_multiplier: constrainedGauss(scaler, 0.01 * jump_multiplier,[0.0, 1.0] ), #eps
                               lambda array,  jump_multiplier: varyVector (array, abs(constrainedGuass(0.0,0.05 * jump_multiplier))), # disk_sym_axis
                               lambda scaler, jump_multiplier: constrainedGauss(scaler, 0.01 * jump_multiplier,[0.1, 1.0] ) ] #lam
