...
class DSphVelocityAnalyzer: 
    ...
    def getRotCoordinates(rot_plane_angle):
        corr_ra = self.corrRa
        corr_dec = self.corrDec
    ...
    def measurePearsonStat (rot_plane_angle, correctVhel = 1):
    
        Vhel, VhelE =  getVelocities(correctVhel)
        dist_along_rot_plane,dist_perp_rot_plane = getRotCoordinates(rot_plane_angle)
    ...
    def velPolyFitAlongAxis (rot_plane_angle, dist_n_bins = 20, fit_order = 1, correctVhel = 1, show_plot = 0):
    ...
    def getCorrOverAngles(angles = np.arange(0,math.pi + math.pi / 256.0, math.pi / 256.0) , fitType = 'pearson', dist_n_bins = 20, fit_order = 1, correctVhel = 1, show_plot = 0):
    
            if fitType.lower() == 'pearson':
                print 'Computing pearson statistic over angles...' 
                stats = [measurePearsonStat (angle, correctVhel = correctVhel) for angle in angles]
            else:
                print 'Computing polynomial fit correlation statistics...'
                stats = [velPolyFitAlongAxis (angle, dist_n_bins = dist_n_bins, fit_order = fit_order, correctVhel = correctVhel, show_plot = 0)
                         for angle in angles]
            if show_plot:
                if fitType.lower() == 'pearson':
                    ...
                    plt.scatter(angles,[stat[0] for stat in stats])
