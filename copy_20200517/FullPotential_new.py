...
class FullPotential:
    def full_value(self,R_halo,z_halo,R_disk,z_disk):
        ...
        halo_coefficient = self.halo_function.getOverallScaleFactor()
    ...
    def potential_value(self, R, z, overall_coefficient, R_scale, z_scale, shape_parameter, potential_function, takeAbs = 1):
        ...
        full_mesh = np.zeros(list(np.shape(coordinate_array)[0:2]) + [3]) + shape_parameter
        full_mesh[:,:,0:2] = coordinate_array 
        
        potential_mesh_new = potential_function.interpolator(full_mesh)
    ...
    def __init__(self,gamma, M, rs,eps, cutoff_correction,c,lam,zeta,el,disk_potential_file = None, halo_potential_file = None, disk_interpolating_function=None, halo_interpolating_function=None, halo_type = 'NFW'):
        ...
        if disk_interpolating_function:
            ...
            self.ffunction = readInPotentialInterpolator('sech_disk') 
        if halo_interpolating_function:
            self.gammafunction=readInPotentialInterpolator(halo_type)
        else:
