...
class MassDensity:
    ...
    def integrate_los(self, halo_sym_axis, disk_sym_axis, integration_bin_centers, apply_tidal_constraint = 0, los_bin_step = 1.0): #,integration_rect_centers):
        ...
        #METHOD 1: ~6.28 s
        #cross_sections_array = self.get_rotated_cross_section(halo_sym_axis, disk_sym_axis, integration_bin_centers)
        #print 'np.shape(cross_sections_array) ='
        #print np.shape(cross_sections_array) 
        #integrated_array = sum([ cross_sections_array[:,:,i] * (integration_bin_borders[i + 1] - integration_bin_borders[i]) for i in range(len(integration_bin_centers))])

        #METHOD 2: ~ 4.5s
        integrated_array = sum([self.get_rotated_cross_section(halo_sym_axis,disk_sym_axis, integration_bin_centers[i])[:,:,0]
                                 * (integration_bin_borders[i + 1] - integration_bin_borders[i]) for i in range(len(integration_bin_centers))])
        #NOTE WE THE TRANSPOSE SINCE THAT IS THE THING THAT WE CAN FEED INTO AN INTERPOLATOR
        print np.shape(integrated_array) 
        integrated_array = np.transpose(integrated_array)
    ...
    def get_rotated_cross_section(self, halo_sym_axis, disk_sym_axis, los_depths):

        #x goes right on sky, z goes up on sky, y goes along los AWAY from observer
        y_sky = los_depths # the distance PAST gal center, alogn los
        xmesh_sky,zmesh_sky,ymesh_sky = np.meshgrid(self.R_values, self.z_values, y_sky)
        ...
        zmesh_halo = xmesh_sky * halo_sym_axis[0] + ymesh_sky * halo_sym_axis[1] + zmesh_sky * halo_sym_axis[2]
        zmesh_disk = xmesh_sky * disk_sym_axis[0] + ymesh_sky * disk_sym_axis[1] + zmesh_sky * disk_sym_axis[2]

        #The R_sym known since R_sky ** 2 + z_sky ** 2 = R_sym ** 2 + z_sym ** 2
        # => R_sym ** 2 = R_sky ** 2 + z_sky **2 - z_sym ** 2
        Rmesh_halo = np.sqrt( xmesh_sky * xmesh_sky + ymesh_sky * ymesh_sky + zmesh_sky * zmesh_sky - zmesh_halo * zmesh_halo )
        Rmesh_disk = np.sqrt( xmesh_sky * xmesh_sky + ymesh_sky * ymesh_sky + zmesh_sky * zmesh_sky - zmesh_disk * zmesh_disk )
