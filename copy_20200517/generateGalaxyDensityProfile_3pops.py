...
def generateGalaxyDensityProfile(lam, zeta, eps, el, theta, a, b, disk_file, halo_file, disk_funct, halo_funct):
    ...
    sigSqr_MP=14**2 #Metal poor stars.  
    sigSqr_IM=11.5**2 #Intermediate metalicity stars
    sigSqr_MR=8.5**2 #Metal rich stars
    ...
    tot_stars=562.0
    typ_mag_MP=(21.2+21.6)/2.0
    tot_stars_MP=185 #Battaglia claims most MP stars are old
    typ_mag_IM=(20.05+20.6)/2.0
    tot_stars_IM=tot_stars*0.4 #Battaglia claims ~40% of all stars are intermediate age
    typ_mag_MR = (19.4+20.8)/2.0
    tot_stars_MR=tot_stars- tot_stars_MP - tot_stars_IM #number of stars table doesn't say what fraction are MR, so assume half and half
    
    #scaling constant of star mass densities.  
    C_MP=10**(typ_mag_MP/-2.5)*tot_stars_MP 
    C_IM=10**(typ_mag_IM/-2.5)*tot_stars_IM
    C_MR=10**(typ_mag_MR/-2.5)*tot_stars_MR
    C_MP, C_IM, C_MR = np.array((C_MP,C_IM,C_MR))/(C_MP+C_IM+C_MR)
    
    
    disk_MR_frac = 0.3 #fraction of young stars in disk
    mass_MR_to_other =3 #relative typical mass of the young stars vs old stars
    eps_stars=(M_star/M) * tot_stars_MR/tot_stars * disk_MR_frac * mass_MR_to_other #fraction of total mass in stellar disk purely due to stars
    ...
    print str(gamma*M/(sigSqr_MP * rs)) + ' is our exponential scaling for MP stars.'
    print str(gamma*M/(sigSqr_IM * rs)) + ' is our exponential scaling for IM stars.'
    print str(gamma*M/(sigSqr_MR * rs)) + ' is our exponential scaling for MR stars.'
    ...
    mass_prof_MP=MassDensity(R,z,gamma,M,rs,eps,f,c,lam,zeta,e,vphi,disk_file,halo_file,C_MP,sigSqr_MP,disk_funct,halo_funct)
    mass_prof_IM=MassDensity(R,z,gamma,M,rs,eps,f,c,lam,zeta,e,vphi,disk_file,halo_file,C_IM,sigSqr_IM,disk_funct,halo_funct)
    mass_prof_MR=MassDensity(R,z,gamma,M,rs,eps,f,c,lam,zeta,e,vphi,disk_file,halo_file,C_MR,sigSqr_MR,disk_funct,halo_funct)
    ...
    los_MP=mass_prof_MP.integrate_los(theta,a,b,x_integration_pnts)
    los_IM=mass_prof_IM.integrate_los(theta,a,b,x_integration_pnts)
    los_MR=mass_prof_MR.integrate_los(theta,a,b,x_integration_pnts)
    los_total = los_MP + los_IM + los_MR
    
    los_profs = {'MR':los_MR,'IM':los_IM,'MP':los_MP}
    
    for pop_of_interest in los_profs.keys() :
    
        zmesh,Rmesh=np.meshgrid(mass_prof_MR.get_z_range(),mass_prof_MP.get_R_range())
        ...
        header='galaxy = ' + galaxy + ' zeta='+str(zeta)+",c="+str(c)+",lambda="+str(lam)+",theta="+str(theta)+",epsilon="+str(eps)+",el="+str(el)+",a="+str(a)+",b="+str(b) + ",pop=" + pop_of_interest
        full_file_dir = '/media/sf_Sasha/Documents/Harvard/physics/randall/dwarfGalaxyProject/simulatedProfileFiles/'
        file_name = 'DGP_' + galaxy + '_' + pop_of_interest + '_zeta_' + str(zeta) + '_lambda_' + str(lam) + '_epsilon_'+ str(eps) + '_el_' + str(el) + '_theta_' + str(theta) + '_a_' + str(a) + '_b_' + str(b) + '.txt'
