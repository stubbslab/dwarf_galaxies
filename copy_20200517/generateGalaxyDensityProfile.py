...
def generateGalaxyDensityProfile(lam, zeta, eps, el, theta, a, b, disk_file, halo_file, disk_funct, halo_funct,plot_save_dir,profile_data_save_dir):
    ...
    potential_dir="/Users/sasha/Documents/Harvard/physics/randall/potential_tables/"
    ...
    log_levels=logList(np.min(np.abs(los_total)),np.max(np.abs(los_total)),20)
    CS=plt.contour(Rmesh,zmesh,los_total,norm=LogNorm(),levels=log_levels)
    ...
    full_file_dir = profile_data_save_dir
