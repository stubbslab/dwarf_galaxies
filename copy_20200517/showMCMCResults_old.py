...
def showMCMCResults(results_file, vars_to_disp, results_dir = '/Users/sasha/Documents/Harvard/physics/randall/MCMCOutputs/' , n_bins = [] ):
    ...
    if len(vars_to_disp) == 2:
        ...
        log_levels = logList(1.0*0.5/total_visits,np.max(np.abs(binned_n_visits + 1.0)),30)
        print log_levels
        ...
        print 'np.shape(binned_n_visits): '
        print np.shape(binned_n_visits)
        CS = plt.contour(x_mesh,y_mesh,binned_n_visits + 1.0,20)
