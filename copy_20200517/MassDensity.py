...
class MassDensity:
    ...
    def get_density_cylindrical(self,R_halo,z_halo,R_disk,z_disk, exponent_scaling):
        ...
        term2_exponent = -self.dispersion_b * (self.potential.full_value(R_halo, z_halo, R_disk, z_disk) - self.potential.full_value(0.0, 0.0, R_disk, z_disk))/self.sigsqr_paramparam + exponent_scaling
        term2 = np.exp(term2_exponent)
        #term3 = np.where(R_halo > 0.0, np.exp(self.beta_funct_over_R_int(R_halo)), 1.0)
        term3 = np.exp(self.beta_funct_over_param_int(R_halo))
