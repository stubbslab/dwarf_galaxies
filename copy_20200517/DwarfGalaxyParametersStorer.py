...
class DwarfGalaxyParametersStorer:
    ...
    def defineBetaIntFunctionForVelocityDispersion(self):
        ...
        print ('beta_scalingg = ' + str(beta_scaling))
    ...
    def getMCMCInfo(self):
        ...
        if self.withDisk:
            params_to_vary = ['M', 'rs', 'halo_sym_axis', 'halo_center', 'el', 'zeta', 'disk_sym_axis', 'disk_center', 'eps',  'lam']#set params to vary in case of disk + halo
            ...
            param_functions = [lambda scaler, jump_multiplier: constrainedGauss (scaler, self.param_step_sizes['M'] * jump_multiplier, self.param_ranges['M'] ), #M
                               ...
                               lambda scaler, jump_multiplier: constrainedGauss(scaler, self.param_step_sizes['zeta'] * jump_multiplier, self.param_ranges['zeta'] ), #zeta
            ...
            params_to_vary = ['M', 'rs', 'halo_sym_axis', 'halo_center', 'el']#set params to vary in case of just halo
    ...
    def printContents(self):
        ...
        print ('self.omegaphi = ' + str(self.omegaphi))
    ...
    def __init__(self, population, withDisk, dist = 'none', M = 'none', vphi = 'none', sigsqr_RR = 'none',
                  el=0.3, rs = 500.0, phi = 'none', theta = 'none', halo_sym_axis = [0.0,0.0,1.0], halo_center = [0.0, 0.0, 0.0], c = 5.0,
                  lam=0.15, zeta = 1.0, eps = 0.05, a = 'none', b = 'none', disk_sym_axis = [0.0,0.0,1.0], disk_center = [0.0, 0.0, 0.0],
                  dispersion_b = 0.0, gamma0 = 0.0, beta_exp = 1.0, beta_R1 = 1.0, beta_R2 = 1.0,
        ...
        if vphi is 'none':
            if stellar_data is None:
                self.omegaphi = data_archive.getRotationalVelocity(population) # angular rotational velocity, in /s {"carina":0.0,"fornax":0.0,"sculptor":0.0,"sextans":0.0}
            else:
                self.omegaphi = stellar_data.measureRotationSignal(self.dist, self.halo_center, self.halo_sym_axis)
        else:
            self.omegaphi = vphi
        ...
        self.param_order_array = ['dist','M', 'omegaphi' , 'sigsqr_RR' ,
                                  'dispersion_b', 'gamma0', 'beta_exp', 'beta_R1','beta_R2',
                                  'el', 'rs' , 'phi' , 'theta' , 'halo_sym_axis', 'halo_center', 'c',
                                  'lam', 'zeta', 'eps', 'a', 'b', 'disk_sym_axis', 'disk_center' ] #The order of the parameters in all_params_ordered array
        #Can get the index of a parameter in the all_params_ordered array using .index('name of parameter')
        self.all_params_ordered = [self.dist, self.M, self.omegaphi , self.sigsqr_RR,
                                   ...
                                   self.lam, self.zeta, self.eps, self.a, self.b, self.disk_sym_axis, self.disk_center ]
        ...
        self.param_step_sizes = {'M':5.0 * 10.0 ** 7.0, 'rs':60.0, 'h_x_center':3.0, 'h_z_center':3.0,
                                 'zeta':0.01, 'el':0.05, 'd_x_center':5.0, 'd_z_center':5.0, 'eps':0.01, 'lam':0.01}
