...
def getNBinsArray():
    n_bins_array = {'M':100, 'el':100, 'rs':300, 'phi':100, 'theta':100, 'h_x_center':100, 'h_y_center':100, 'h_z_center':100,
                    'lam':100, 'Rd':100, 'eps':100, 'a':100,'b':100, 'd_x_center':100, 'd_y_center':100, 'd_z_center':100}
    return n_bins_array

def getParamDisplayVals():
    n_bins_array = getNBinsArray()
    r_half_light = 791.0
    M_star = 10.0 ** 7.39 
    param_display_vals = {'M':[1.0 * M_star, 400.0 * M_star, n_bins_array['M']],
                          'el':[0.1,10.0,n_bins_array['el']],
                          'rs':[0.1 * r_half_light, 10.0 * r_half_light, n_bins_array['rs']],
                          'phi':[0.0,math.pi, n_bins_array['phi']],
                          'theta':[-math.pi / 2.0 , math.pi / 2.0, n_bins_array['theta']],
                          'h_x_center':[-r_half_light, r_half_light, n_bins_array['h_x_center']],
                          'h_y_center':[-r_half_light, r_half_light, n_bins_array['h_y_center']],
                          'h_z_center':[-r_half_light, r_half_light, n_bins_array['h_z_center']],
                          'lam':[0.05,0.15,n_bins_array['lam']],
                          'Rd':[0.1 * r_half_light, 10.0 * r_half_light, n_bins_array['Rd']],
                          'eps':[0.0,0.5,n_bins_array['eps']],
                          'a':[0.0,math.pi ,n_bins_array['a']],
                          'b':[-math.pi / 2.0, math.pi / 2.0, n_bins_array['b']],
                          'd_x_center':[-r_half_light, r_half_light, n_bins_array['d_x_center']],
                          'd_y_center':[-r_half_light, r_half_light, n_bins_array['d_y_center']],
                          'd_z_center':[-r_half_light, r_half_light, n_bins_array['d_z_center']]
                          }
...
def getVarTicks():
    ...
    var_ticks = {'M' : [1000, 3000, 5000, 7000, 9000], 'el':[10.0 ** -0.6, 10.0 ** -0.0, 10.0 ** 0.6], 'rs':[1000, 3000, 5000, 7000],
                 'phi':[math.pi / 4.0, math.pi / 2.0, 3.0 * math.pi / 4.0], 'theta':[math.pi / 4.0, math.pi / 2.0, 3.0 * math.pi / 4.0], 
                 'h_x_center':[-500, 0, 500], 'h_y_center':[-500, 0, 500], 'h_z_center':[-500, 0, 500],
                 'lam':[0.11, 0.13, 0.15, 0.17, 0.19], 'Rd':[1000, 3000, 5000, 7000], 'eps':[0.15, 0.35],
                 'a':np.around([math.pi / 4.0, math.pi / 2.0, 3.0 * math.pi / 4.0],3), 'b':np.around([math.pi / 4.0, math.pi / 2.0, 3.0 * math.pi / 4.0],3), 'd_x_center':[-500, 0, 500], 'd_y_center':[-500, 0, 500], 'd_z_center':[-500, 0, 500]}
    return var_ticks

def getVarTickLabels():
    var_ticklabels = {'M' : ['1','3','5','7','9'], 'el':[r'$-0.6$', r'$0$', r'$0.6$'], 'rs':['1',  '3',  '5', '7'],
                 'phi':['45', '90', '135'], 'theta':['45', '90', '135'],
                 'h_x_center':[r'$-500$', '0', '500'], 'h_y_center':[r'$-500$', '0', '500'], 'h_z_center':[r'$-500$', '0', '500'],
                 'lam':['0.11', '0.13', '0.15', '0.17', '0.19'], 'Rd':['1',  '3',  '5', '7'], 'eps':['0.15', '0.35'],
                 'a':['45', '90', '135'], 'b':['45', '90', '135'], 'd_x_center':[r'$-500$', '0', '500'], 'd_y_center':[r'$-500$', '0', '500'], 'd_z_center':[r'$-500$', '0', '500'] }
    return var_ticklabels

def getBinTypes():
    bin_types = {'M' : 'lin', 'el':'log', 'rs':'lin', 'phi':'lin', 'theta':'lin', 'h_x_center':'lin', 'h_y_center':'lin', 'h_z_center':'lin', 'lam':'lin', 'Rd':'lin', 'eps':'lin','a':'lin','b':'lin', 'd_x_center':'lin', 'd_y_center':'lin', 'd_z_center':'lin' }
    return bin_types

def getDisplayLabels():
    disp_labels = {'M':r'$M$ ' + r'(' + '$10$' + r'$^9$' + ' ' + r'$\mathrm{M}$' + r'$_\odot$' + ')', 'el':'$\mathrm{log}_{10} (Q)$', 'rs':'$r_s$ ' + ' $(\mathrm{kpc})$', 'phi':r'$\theta$' + ' ' + r'$(\mathrm{deg})$', 'theta':r'$\phi$' + ' ' + r'$(\mathrm{deg})$',
                      'h_x_center':r'$h_c$' + r'$_,$' + r'$_x$' + ' $(\mathrm{pc})$', 'h_y_center':r'$h_c$' + r'$_,$' + r'$_y$' + ' $(\mathrm{pc})$',  'h_z_center':r'$h_c$' + r'$_,$' + r'$_z$' + ' $(\mathrm{pc})$',
                      'lam':r'$\lambda$', 'Rd':'$R_d$ ' + ' $(\mathrm{kpc})$', 'eps':r'$\epsilon$','a':'$a$' + ' (deg)','b':'$b$' + ' (deg)',
                      'd_x_center':r'$d_c$' + r'$_,$' + r'$_x$' + ' $(\mathrm{pc})$', 'd_y_center':r'$d_c$' + r'$_,$' + r'$_y$' + ' $(\mathrm{pc})$',  'd_z_center':r'$d_c$' + r'$_,$' + r'$_z$' + ' $(\mathrm{pc})$',}
    return disp_labels 

def getDispUnits():
    var_units = {'M' : ' (sol M)', 'el':'', 'rs':'', 'phi':'', 'theta':'', 'h_x_center':'', 'h_y_center':'', 'h_z_center':'',
                 'lam':'', 'Rd':'', 'eps':'','a':'','b':'', 'd_x_center':'', 'd_y_center':'', 'd_z_center':''}
    return var_units

def getVarCyclicity():
    var_cyclicity = {'M' : 'none', 'el':'none', 'rs':'none', 'phi':'none', 'theta':'none', 'h_x_center':'none', 'h_y_center':'none', 'h_z_center':'none',
                     'lam':'none', 'Rd':'none', 'eps':'none', 'a':'none', 'b':'none', 'd_x_center':'none', 'd_y_center':'none', 'd_z_center':'none' }
    return var_cyclicity 

def getVarIndeces():
    var_indeces = {'M' : 12, 'rs':13, 'phi':17, 'theta':18, 'h_x_center':19, 'h_y_center':20, 'h_z_center':21, 
                   'Rd':22, 'el':23, 'eps':24, 'a': 28, 'b':29, 'd_x_center':30, 'd_y_center':31, 'd_z_center':32, 'lam':33}
    return var_indeces

def getVarDisplayScalings():
    var_display_scalings = {'M' : 10.0 ** (-6.0), 'el':1.0, 'rs':1.0, 'phi':1.0, 'theta':1.0, 'h_x_center':1.0, 'h_y_center':1.0, 'h_z_center':1.0,
                            'lam':1.0, 'Rd':1.0, 'eps':1.0, 'a':1.0, 'b':1.0, 'd_x_center':1.0, 'd_y_center':1.0, 'd_z_center':1.0 }
...
def measureMCMCResults(results_files, vars_to_fit, fitting_functions, guess_params_set, function_to_fit = [], 
                       results_dir = '/Users/sasha/Documents/Harvard/physics/randall/MCMCOutputs/', n_ignore = 0, n_bins_set = [200],
                       show_fit = 1, ranges_to_fit = ['all'], bin_buffer = 0, maxfev = 2000, bounds = [(-np.inf, np.inf)], max_n_sigma_right = 0.1):
    ...
    for i in range(len(vars_to_fit)):
        ...
        if fitting_function is 'single_hump':
            ...
            fitting_function = lambda xs, A, mu, sigma, shift: A * np.exp(-(xs - mu)**2.0 /(2.0 * sigma ** 2.0)) + shift
        ...
        fit_results_set = fit_results_set + [results]
        if show_fit:
            plt.scatter(x_vals, y_vals)
    ...
    recomputed_params = recompute_param_function(fit_results_set[0][0])
...
def fitMCMCResults(results_files, vars_to_fit,
                   measured_arrays = None, n_visits_array = None,
                   ...
                   results_dir = '/Users/sasha/Documents/Harvard/physics/randall/MCMCOutputs/', fixed_slope = None, 
...
def showMCMCResults(results_files, vars_to_disp,
                    ...
                    results_dir = '/Users/sasha/Documents/Harvard/physics/randall/MCMCOutputs/', file_name = '', n_x_ticks = 5, n_y_ticks = 5,
                    n_bins = [], n_ignore = 0, theta_shift = 0.0, n_var_per_plot = 1, fig_size_unit = 5.0, n_levels = 10, exp_level_powers = [-3.0, -2.0, -1.0], 
    ...
    disp_units = getDispUnits() 
    display_labels = getDisplayLabels()
        ...
        for i in range(len(vars_to_disp) - 1):
            ...
            for j in range(i, len(vars_to_disp) - 1):
                ...
                if n_var_per_plot is 'both':
                    ...
                    axarr[axarr_y_index][0].set_yticklabels(yticklabels, fontsize = 10.0 * fig_size_unit * n_y_contour_plots/ (n_y_ticks), rotation = 0)
                else:
                    axarr[axarr_y_index][axarr_x_index].set_xticklabels([])
    ...
    if n_var_per_plot == 1 or n_var_per_plot in ['both','Both','BOTH']:
        ...
        for i in range(len(vars_to_disp)):
            ...
            if n_var_per_plot == 1:
               ...
               axarr[i % n_x_plots][i // n_x_plots].set_xlabel(xlabel)
            else:
                x_index = i + 1
                y_index = i - 1
    ...
    if save_fig:
        if file_name is '': file_name = 'MCMC_results_plots_vars_' + '_'.join([str(elem) for elem in vars_to_disp]) + '_' + str(n_var_per_plot) + '_per_plot' + '.pdf'
...
def fitCurveToMCMCResults(results_files,  poly_order = 2, fitting_funct = 'poly', ind_var = 'rs', dep_var = 'M', 
                              results_dir = '/Users/sasha/Documents/Harvard/physics/randall/MCMCOutputs/', n_ignore = 0, n_fit_points = 200,
