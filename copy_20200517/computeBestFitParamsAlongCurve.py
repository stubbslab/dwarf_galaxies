...
def computeLikelihoodGivenSeedAndFixedParams(galaxy, withDisk, seed_params, seed_x, seed_y,
                                             ...
                                             fixed_hx_rs_val, fixed_hz_rs_val, fixed_dx_rs_val, fixed_dz_rs_val, star_data_sets, apply_rotation):
    ...
    for i in range(len(pops)):
        ...
        storer =  DwarfGalaxyParametersStorer(population, withDisk = withDisk, sigsqr = star_data.sigSqr, 
                                              ...
                                              halo_sym_axis = param_initializer['halo_sym_axis'], halo_center = param_initializer['halo_center'],
                                              lam = param_initializer['lam'], zeta = param_initializer['Rd'] / param_initializer['rs'], eps = param_initializer['eps'],
...
def computeBestFitParamsAlongCurve(results_files, galaxy, withDisk, fixed_params, halo_type, disk_type,
                                   ...
                                   curve_params = ['rs','M'], n_xs_to_check = 10, x_range_to_check = [50.0, 10000.0], y_range_to_check = [10.0 ** 7.0, 10.0 ** 10.0], max_refine_steps = 6, max_final_steps = 100, outer_step = 1.0, show = 1, apply_rotation = 1, 
    ...
    default_params = {'halo_sym_axis':None, 'halo_y_center':0.0, 'theta':0.0, 
                      'eps':0.0, 'lam':0.1, 'Rd':1000.0, 'a':0.0, 'b':0.0, 'disk_sym_axis':None,
                      'd_x_center':0.0, 'd_y_center':0.0, 'd_z_center':0.0  }
