...
def readInBootstrapResults(bootstrap_files, params_to_read_in = ['el', 'phi', 'h_x_center', 'rs', 'M', 'h_z_center']):
    n_items_per_halo = 3
    name_index = 0
    ...
    n_halos = len(first_results) // n_items_per_halo 
    ...
    all_results = [np.load(load_file, encoding = 'latin1') for load_file in bootstrap_files]
    ...
    for run_results in all_results:
        for single_results_set in run_results:
            for i in range(n_halos):
                ...
                for param in params_to_read_in:
                    dicts_by_halo[halo][param] = dicts_by_halo[halo][param] + [fitted_params[param] ]
                dicts_by_halo[halo]['likelihood'] = dicts_by_halo[halo]['likelihood'] + [likelihood]
