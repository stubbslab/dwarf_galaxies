...
def doBootstrapOnMCMC(galaxy, model_types, halo_types, disk_types, halo_functs, disk_functs, doWithDisksSet, param_sets_to_measure,
                      n_resamples = 'full', w_replace = 1, pop_selection_method = 'metal_rigid', save_runs = 1, disk_MCMC_multiplier = 1,
                      specified_param_ranges_set = [{'a':0.0}], save_res = 1, n_interim_iters_to_save = 1000,
                      ...
                      curve_param_box_size = [3000.0, 1.0 * 10.0 ** 9.0], n_points_along_curve_to_find_max = 50, pop_independent_params = ['M', 'rs', 'halo_center'],
                      smallest_max_val_to_be_on_curve = 50.0, show_results = 0, randomization_file_name = '' , n_single_param_bins = 30,
                      do_randomization = 1, pop_key_strs = ['_MP','_IM','_MR']):

    disk_MCMC_multiplier = int(disk_MCMC_multiplier)
    res_sets = []
    model_to_start_index = {'nfw_obl':0, 'nfw_pro':1, 'burkert_obl':2, 'burkert_pro':3}
    ...
    while n_successful_computations < n_randomizations:
        ...
        for j in range(len(model_types)):
            ...
            [runMCMCForDwarfGalaxyProfilesAllPops(galaxy, withDisk = doWithDisksSet[j], pop_selection_method = pop_selection_method,
                                                  disk_funct = disk_funct, halo_funct = halo_funct, h_type = halo_type, d_type = disk_type,
                                                  nIterations = n_MCMC_iterations,
                                                  start_param_index = model_to_start_index[model_type], halo_edge_on = 1, disk_edge_on = 1,
                                                  saverun = save_runs,
                                                  mask_R_bins = mask_R_bins, mask_z_bins = mask_z_bins, max_sky_angle_deg = max_sky_angle_deg, target_n_sky_pixels = target_n_sky_pix, n_los_bins = n_los_bins,
                                                  star_data = resampled_stars, include_morph_prob = include_morph_prob, include_kinem_prob = include_kinem_prob,
            ...
            WOD_MCMC_files = ['MCMCResultsFromResampledStars/MCMC_out_' + extra_save_str + 'randomization' + str(n_successful_computations+extra_iterator_save_number) + '_chain' + str(chain) + halo_type + '_' + disk_type + '_WD_0_' + prolate_or_oblate + '_start_'
                              + str(model_to_start_index[model_type]) + '_fornax_simul_PSM_metal_rigid_HEdge_1_DEdge_1_fixDA_Nonepi_fixHA_Nonepi__crowdMsk_0_N_'
            ...
            try:
                if show_results:
                    memr.showMCMCResults(WOD_MCMC_files, ['rs','M','el','phi','h_x_center','h_z_center'], n_var_per_plot = 2, fig_size_unit = 2.5, params_to_fit = {'rs':'buffer','M':'buffer'}, smallest_max_val_to_be_on_curve = smallest_max_val_to_be_on_curve, n_fitted_points = n_points_along_curve_to_find_max)
                print ('Trying to measure best fit for files ' + str(WOD_MCMC_files) )
                #best_fit_vals_dict = {param_to_measure: memr.measureMCMCResults(WOD_MCMC_files, [param_to_measure], ['single_hump_shift'], ['single_hump_shift'], n_bins_set = [n_single_param_bins], show_fit = show_results)[0][1]
                #                      for param_to_measure in params_to_measure}
                #best_fit_vals_dict_set = best_fit_vals_dict_set + [best_fit_vals_dict]
                best_fit_vals_dict = {}
                for param_to_measure in params_to_measure:
                    full_fit_results_for_pop_dependent_param = memr.measureMCMCResults(WOD_MCMC_files, [param_to_measure], ['single_hump_shift'], ['single_hump_shift'], n_bins_set = [n_single_param_bins], show_fit = show_results, n_ignore = int(n_MCMC_iterations * n_ignore_frac), withDisk = 1)
                ...
                curve_fitting_results = computeBestFitParamsAlongCurve(WOD_MCMC_files, 'fornax', 0, best_fit_vals_dict, halo_type, disk_type, n_los_bins, target_n_sky_pix, mask_R_bins,
                                                                       ...
                                                                       n_fitted_points_along_curve = n_points_along_curve_to_find_max,
                ...
                print ('MCMC_WOD_results_sets now: ' + str(MCMC_WOD_results_sets))
        ...
        if len(best_fit_vals_dict_set) == len(model_types):
            ...
            for j in range(len(model_types)):
                ...
                if doWithDisk:
                    ...
                    MCMC_WD_results_sets = MCMC_WD_results_sets + [runMCMCForDwarfGalaxyProfilesAllPops(galaxy, withDisk = withDisk , disk_funct = disk_funct, halo_funct = halo_funct,
