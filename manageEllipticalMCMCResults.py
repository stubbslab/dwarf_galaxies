...
def getNBinsArray(pops = [], pop_connector = '_'):
    n_bins_array = {'M':200, 'rs':200, 'h_x_center':200, 'h_y_center':200, 'h_z_center':200, 'el':200,'phi':100,'theta':100,
                    'eps':50, 'Rd':200, 'lam':100, 'd_x_center':200, 'd_z_center':200,
                    'sigsqr_RR':100, 'omega_phi':100, 'sigsqr_rr_inf_to_0_rat':100, 'r_sigsqr_RR':100, 'alpha_sigsqr_rr':100, 'gamma_for_beta_inf':100, 'r_beta0':100, 'alpha_beta':100,
                    'sigsqr_rr_0_IM':100, 'sigsqr_rr_inf_to_0_rat_IM':100, 'r_sigsqr_rr0_IM':100, 'alpha_sigsqr_rr_IM':100, 'gamma_for_beta_inf_IM':100, 'r_beta0_IM':100, 'alpha_beta_IM':100,
                    'sigsqr_rr_0_MR':100, 'sigsqr_rr_inf_to_0_rat_MR':100, 'r_sigsqr_rr0_MR':100, 'alpha_sigsqr_rr_MR':100, 'gamma_for_beta_inf_MR':100, 'r_beta0_MR':100, 'alpha_beta_MR':100, }
...
def getParamDisplayVals(pops = [], pop_connector = '_'):
    ...
    print ('list(bg_storer.param_ranges.keys()) = ' + str(list(bg_storer.param_ranges.keys())))
    print ('list(n_bins_array.keys()) = ' + str(list(n_bins_array.keys())))
    r_half_light = 791.0
    M_star = 10.0 ** 7.39
    param_display_vals = {'M':[*bg_storer.param_ranges['M'], n_bins_array['M']],
                          'rs':[*bg_storer.param_ranges['rs'], n_bins_array['rs']],
                          'el':[*bg_storer.param_ranges['el'], n_bins_array['el']],
                          'phi':[0.0, np.pi, n_bins_array['phi']],
                          'theta':[0.0, 2.0 * np.pi, n_bins_array['theta']],
                          ...
                          'sigsqr_RR':[*bg_storer.param_ranges['sigsqr_RR'], n_bins_array['sigsqr_RR']],
                          'omega_phi':[*bg_storer.param_ranges['omega_phi'], n_bins_array['omega_phi']],
                          'eps':[*bg_storer.param_ranges['eps'], n_bins_array['eps']],
                          'lam':[*bg_storer.param_ranges['lam'], n_bins_array['lam']],
                          'd_x_center':[*bg_storer.param_ranges['d_x_center'], n_bins_array['d_x_center']],
                          'd_z_center':[*bg_storer.param_ranges['d_z_center'], n_bins_array['d_z_center']],
                          'Rd':[*bg_storer.param_ranges['Rd'], n_bins_array['Rd']]
                          }
...
def getVarTicks(pops = [], pop_connector = '_'):
    ...
    var_ticks = {'M' : [2000, 7000, 12000], 'rs':[2000, 4000, 6000], 'el':[0.2, 1.0, 5.0],
                 'theta':[np.pi / 3.0, np.pi, 5.0 * np.pi / 3.0], 'phi':[np.pi / 6.0, 5.0 * np.pi / 6.0],
                 'h_x_center':[-0.2, 0.2], 'h_y_center':[-0.2, 0.2], 'h_z_center':[-0.2, 0.2],
                 'sigsqr_RR':[100.0,  300.0],  'omega_phi':[-6.0 / Gyr_to_s, 0.0, 6.0 / Gyr_to_s], 'r_sigsqr_rr0':[1000, 3000, 5000, 7000], 'alpha_sigsqr_rr':[1.5, 2.5, 3.5, 4.5],
                 'eps':[0.1, 0.4], 'd_x_center':[-0.2, 0.2] , 'd_z_center':[-0.2, 0.2], 'lam':[0.05, 0.1, 0.15], 'Rd':[2000, 4000, 6000],
                 'gamma_for_beta_inf':[-0.5, 0.0, 0.5], 'r_beta0':[1000, 3000, 5000, 7000], 'alpha_beta':[1.5, 3.0, 4.5]}
...
def getVarTickLabels(pops = [], pop_connector = '_'):
    var_ticklabels = {'M' : ['2', '7', '12'], 'rs':['2', '4', '6'], 'el':['0.2', '1', '5'], 'theta':[r'$\pi /3$', r'$5 \pi /3$'], 'phi':[r'$\pi /6$', r'$5 \pi /6$', ],
                 'h_x_center':[r'$-0.2$', '0.2'], 'h_y_center':[r'$-0.2$', '0.2'], 'h_z_center':[r'$-0.2$', '0.2'],
                 'sigsqr_RR':['100','300'],  'omega_phi':[r'$-6$', '0', '6'], 'r_sigsqr_rr0':['1',  '3',  '5', '7'], 'alpha_sigsqr_rr':['1.5', '2.5', '3.5', '4.5'],
                 'eps':['0.1', '0.4'], 'd_x_center':[r'$-0.2$',  '0.2'] , 'd_z_center':[r'$-0.2$', '0.2'], 'lam':[0.05, 0.1, 0.15], 'Rd':['2', '4', '6'],
                 'gamma_for_beta_inf':[r'$-0.5$', r'$0$', '0.5'], 'r_beta0':['1',  '3',  '5', '7'], 'alpha_beta':['1.5', '3', '4.5']}
...
def getBinTypes(pops = [], pop_connector = '_'):
    bin_types = {'M' : 'lin', 'rs':'lin', 'h_x_center':'lin', 'h_y_center':'lin', 'h_z_center':'lin',
                 'el':'log', 'theta':'lin', 'phi':'lin',
                 'sigsqr_RR':'lin', 'omega_phi':'lin', 'sigsqr_rr_inf_to_0_rat':'log', 'r_sigsqr_rr0':'lon', 'alpha_sigsqr_rr':'lin',
                 'eps':'lin', 'd_x_center':'lin' , 'd_z_center':'lin', 'lam':'lin', 'Rd':'lin',
...
def getDisplayLabels(pops = [], pop_connector = '_', pop_additional_labels = {r'$MP$', r'$IM$', r'$MR$'}):
    disp_labels = {'M':r'$M$ ', 'el':r'$\mathrm{log}_{10} (Q)$', 'rs':'$r_s$ ' , 'phi':r'$\theta$' , 'theta':r'$\phi$' ,
                      'h_x_center':r'$h_c$' + r'$_,$' + r'$_x$' , 'h_y_center':r'$h_c$' + r'$_,$' + r'$_y$' ,  'h_z_center':r'$h_c$' + r'$_,$' + r'$_z$' ,
                      'sigsqr_RR':r'$\sigma_{rr,0}$', 'omega_phi':r'$\omega$','sigsqr_rr_inf_to_0_rat':r'km/s$^{-1}$', 'r_sigsqr_rr0':r'$r_{\sigma, rr}$','alpha_sigsqr_rr':r'$\alpha_{\sigma, rr}$',
                      'eps':r'$\epsilon$', 'd_x_center':r'$d_c$' + r'$_,$' + r'$_x$' , 'd_z_center':r'$d_c$' + r'$_,$' + r'$_z$', 'lam':r'$\lambda$', 'Rd':r'$R_d$',
...
def getDispUnits(pops = [], pop_connector = '_'):
    var_units = {'M' : r'$10^9$' + ' ' + r'$M_\odot$', 'rs':'kpc', 'h_x_center':'deg', 'h_y_center':'deg', 'h_z_center':'deg', 'el':'', 'phi':'rad','theta':'deg',
                 'sigsqr_RR':'km/s', 'omega_phi':r'Gyr$^{-1}$', 'sigsqr_rr_inf_to_0_rat':'', 'r_sigsqr_rr0':'kpc', 'alpha_sigsqr_rr':'',
                 'eps':'', 'd_x_center':'', 'd_z_center':'', 'lam':'', 'Rd':'',
...
def getVarCyclicity(pops = [], pop_connector = '_'):
    var_cyclicity = {'M' : 'none', 'rs':'none', 'h_x_center':'none', 'h_y_center':'none', 'h_z_center':'none',
                     'el':'none','phi':'none','theta':'none',
                     'sigsqr_RR':'none', 'omega_phi':'none', 'sigsqr_rr_inf_to_0_rat':'none', 'r_sigsqr_rr0':'none', 'alpha_sigsqr_rr':'none',
                     'eps':'none', 'd_x_center':'none', 'd_z_center':'none', 'lam':'none', 'Rd':'none',
...
def getVarIndeces():
    var_indeces = {'M' : 13, 'rs':14, 'el':15,'phi':19,'theta':20,'h_x_center':21, 'h_y_center':22, 'h_z_center':23,
                   'sigsqr_RR':{'MP':24,'IM':27,'MR':30}, 'omega_phi':{'MP':25,'IM':28,'MR':31},
                   'lam':33, 'Rd':34, 'eps':35, 'a':36, 'b':37, 'd_x_center':41, 'd_y_center':42, 'd_z_center':43,
                   'r_sigsqr_rr0':{'MP':21,'IM':28,'MR':35}, 'alpha_sigsqr_rr':{'MP':22,'IM':29,'MR':36},
                   'gamma_for_beta_inf':{'MP':23,'IM':30,'MR':37}, 'r_beta0':{'MP':24,'IM':31,'MR':38}, 'alpha_beta':{'MP':25,'IM':32,'MR':39}
                   }
    return var_indeces

def getVarPopDependence():
    pop_dependent_vars = ['sigsqr_RR', 'omega_phi', ]
    return pop_dependent_vars

def getVarDisplayScalings(pops = [], pop_connector = '_'):
    Gyr_to_s = 365.2425 * 24 * 3600 * 10.0 ** 9.0
    var_display_scalings = {'M' : 10.0 ** (-6.0),'rs':1.0, 'phi':1.0, 'theta':1.0, 'h_x_center':1.0, 'h_y_center':1.0, 'h_z_center':1.0, 'el':1.0,
                            'sigsqr_RR':1.0, 'omega_phi':1.0, 'sigsqr_rr_inf_to_0_rat':1.0, 'r_sigsqr_rr0':1.0, 'alpha_sigsqr_rr':1.0,
                            'eps':1.0, 'd_x_center':1.0, 'd_z_center':1.0, 'lam':1.0, 'Rd':1.0,
        ...
        with open(data_file) as csvfile:
            ...
            for row in myreader:
                if skip_first_line:
                    ...
                    new_n_visits = int(float((row[n_visits_index]).strip()) )
...
def measureMCMCResults(results_files, vars_to_fit, fitting_functions, guess_params_set,
                       function_to_fit = [], withDisk = 0,
                       ...
                       show_fit = 1, ranges_to_fit = ['all'], bin_buffer = 4, maxfev = 2000, bounds = [(-np.inf, np.inf)], max_n_sigma_right = 0.1):
    ...
    measured_arrays, n_visits_array, likelihood_array = readInMCMCResults(results_files, withDisk = withDisk, functions_to_compute = function_to_fit, vars_to_load = vars_to_read_in,
    ...
    for i in range(len(vars_to_fit)):
        ...
        if 'omega_phi' in var_to_fit:
            Gyr_to_s = 365.2425 * 24 * 3600 * 10.0 ** 9.0
            param_range = [param_range[0] * Gyr_to_s, param_range[1] * Gyr_to_s]
        ...
        if 'omega_phi' in var_to_fit:
            Gyr_to_s = 365.2425 * 24 * 3600 * 10.0 ** 9.0
            print ('Here1 ')
            x_vals = [x_val * Gyr_to_s for x_val in x_vals]
        ...
        if fitting_function is 'single_hump':
            ...
            single_bounds = ([0.0, param_range[0], 0.0], [np.inf, param_range[1], (param_range[1] - param_range[0]) / 2.0])
            ...
            single_bounds = ([0.0, param_range[0],  0.0, 1.0, -np.inf], [np.inf, param_range[1], (param_range[1] - param_range[0]) / 1.0, 10.0, np.inf])
        ...
        if guess_params is 'single_hump':
            ...
            sigma = (param_range[1] - param_range[0]) / 10.0
    ...
    if 'omega_phi' in var_to_fit:
        print ('Correcting output for omega_phi...')
        Gyr_to_s = 365.2425 * 24 * 3600 * 10.0 ** 9.0
        for pop_index in range(len(fit_results_set)):
            fit_results_set[pop_index][1] = fit_results_set[pop_index][1] / Gyr_to_s
            fit_results_set[pop_index][2] = fit_results_set[pop_index][2] / Gyr_to_s
        print ('fit_results_set = ' + str(fit_results_set) )
...
def fitMCMCResults(results_files, vars_to_fit,
                   withDisk = 0, pops = ['MP','IM','MR'], measured_arrays = None, n_visits_array = None,
    ...
    if (measured_arrays is None) or (n_visits_array is None):
        measured_arrays, n_visits_array, likelihood_array = readInMCMCResults(results_files, withDisk = withDisk, vars_to_load = vars_to_fit, results_dir = results_dir, n_ignore = n_ignore )
...
def showMCMCResults(results_files, vars_to_disp,
                    withDisk = 0, params_to_fit = {}, fit_funct = 'poly', params_for_fit = {'fit_order':3, 'deriv_of_fit':None, 'test_params':[], 'init_guess':[], 'bounds':None, 'use_fixed_point':1},
                    pops = ['MP','IM','MR'], pop_connector = '_', pop_additional_labels = [r'$_{,MP}$', r'$_{,IM}$',r'$_{,MR}$'],
                    ...
                    y_label_coord_scalings = [-0.2, 0.5]):
    ...
    measured_arrays, n_visits_array, likelihood_array = readInMCMCResults(results_files, withDisk = withDisk, functions_to_compute = functions_to_display, vars_to_load = vars_to_read_in,
                                                                                         results_dir = results_dir, n_ignore = n_ignore )
    print ('measured_arrays.keys() = ' + str(measured_arrays.keys() ))
    pop_dependent_vars = getVarPopDependence()
    replace_vars_to_disp = []
        ...
        for i in range(len(vars_to_disp) - 1):
            ...
            for j in range(i, len(vars_to_disp) - 1):
                ...
                if n_var_per_plot is 'both':
                    ...
                    axarr[axarr_y_index][0].set_ylabel(ylabel, fontsize = label_size_scaling * (fig_size_unit), rotation = 90)
...
def fitCurveToMCMCResults(results_files,
                           withDisk = 0, poly_order = 2, fitting_funct = 'poly', ind_var = 'rs', dep_var = 'M',
    ...
    measured_arrays, n_visits_array, likelihood_array = readInMCMCResults(results_files, withDisk = withDisk, vars_to_load = vars_to_read_in,
                                                                          results_dir = results_dir, n_ignore = n_ignore )
