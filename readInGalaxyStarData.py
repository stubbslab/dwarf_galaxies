...
def readInGalaxyStarData(population, pop_selection_method='none', plot='no', plot_metallicity='no', save_fig = 0):
    ...
    lines = cant.readInFileLineByLine(data_file)
    for line in lines:
    ...
    into_sky_components_of_Vhel = [corr_Vhel[i] / np.cos(cant.measureAngularSeparationOnSky([0.0, 0.0], [corr_ra[i], corr_dec[i]], return_radian = 1)) for i in range(len(corr_Vhel))] #If the gal was a rigid object, this would measure the motion away from the observer of this object.
    approx_gal_sky_motion = np.mean(into_sky_components_of_Vhel)
    corr_Vhel = [corr_Vhel[i] - approx_gal_sky_motion * np.cos(cant.measureAngularSeparationOnSky([0.0, 0.0], [corr_ra[i], corr_dec[i]], return_radian = 1)) for i in range(len(corr_Vhel))]
